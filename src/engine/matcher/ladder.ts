...
import type { AlignmentOutcome } from "./framework.ts";
...
interface TierProposal {
	readonly pairings: readonly CandidatePairing[];
}
...
const runLadder = (input: LadderInput): LadderResult => {
	...
	const placed: CandidatePairing[] = [];
	for (const rung of rungs) {
		const proposal = rung.propose({ budget, left, placed, right });
		contributions.push({ pairings: proposal.pairings, tier: rung.id });
		placed.push(...proposal.pairings);
	}
	return {
		budget: budget.snapshot(),
		contributions,
		outcome: alignStreams(left, right, placed),
	};
};
