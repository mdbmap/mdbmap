...
const first = anime("a", { anidb: "1", mal: "100" }, [
	{ kind: "sequel", toId: "b" },
]);
...
const last = anime("c", { anidb: "3", mal: "300" }, [
	{ kind: "prequel", toId: "b" },
]);
...
describe("discovery broker", () => {
	it("walks the chain from a supported id and rebases on the earliest entry", async () => {
		...
		expect(
			outcome.chain.segments.map((segment) => segment.entry.id),
		).toStrictEqual(["a", "b", "c"]);
	...
	it("re-fetches the full entry before walking so a relationless search hit still expands", async () => {
		...
		const simkl = clientOver([first, middle, last], {
			findByExternalId: async (service, serviceId) => {
				await Promise.resolve();
				return service === "tmdb" && serviceId === "999"
					? searchHit
					: undefined;
			},
		});
	...
	it("walks a mainline movie sequel from an anime start", async () => {
		const series = anime("a", { anidb: "1", tmdb: "999" }, [
		...
		const simkl = clientOver([series, film]);
		...
		expect(
			outcome.chain.segments.map((segment) => segment.entry.id),
		).toStrictEqual(["a", "film"]);
		expect(
			outcome.chain.segments.map((segment) => segment.entry.type),
		).toStrictEqual(["anime", "movie"]);
	});
