...
Personal media tracker built on a cross-service metadata matcher. TV, film and anime first, then music and books.

- **Stack**: TanStack Start on Cloudflare Workers, D1 + Drizzle, Bun, VoidZero / Vite tooling
- **UI**: React Aria, Tailwind + Tailwind Variants, Lucide
- **Authentication**: Better-Auth
- **Data handling**: T3 Env, Immer, Zod, Zustand, TanStack / React Query, oRPC
- **i18n**: Paraglide
- **Telemetry & logging**: Posthog

## Ensuring code quality

We have a very strict `tsconfig.json` and `oxlint.config.ts`. Do **NOT** edit them unless there are genuinely conflicting rules. Do **NOT** disable linting rules inline until you've exhausted all options, and flag that you did it.

Creating a pull request will invoke an agent to review it, you need to babysit the PR until it gives the all clear.

If you leave a comment, do not explain the "why". It's not a changelog and it's not a report. 99% of the time, that information is never needed again. Do **NOT** leave a comment at all if the code's naming makes it self-explanatory. Also do **NOT** bloat README.md or other markdown files with this information, if you really think it's that important, put it in an unstaged new MD file and I will add it if I think it's worthwhile.

## Agent skills

### Issue tracker

Issues live on GitHub (theacrat/mdbmap) via the `gh` CLI. See `docs/agents/issue-tracker.md`.
